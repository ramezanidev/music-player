...
const getData = async (url: string) => {
    ...
    try {
        ...
        result.songs = data.songs.results
...
getData('https://api-beta.melobit.com/v1/album/' + route.params.id)
// -------------------------
...
<template>
    <div class="p-4 h-full box-border">
        <header class="flex items-center justify-between">
            <div class="flex items-center">
                ...
                <p class="ml-2 text-white">
                    <mark class="bg-transparent text-[#fcab14]">{{ result.name }}</mark> Album
                </p>
            </div>
        </header>
        <div class="flex h-[calc(100%-2rem)] w-full">
            ...
            <div v-if="loading" class="flex flex-col w-full gap-4 animate-pulse overflow-auto pt-8 scroll-smooth">
                <div v-for="i in 8" :key="i" class="w-full h-16 flex">
                    <div class="bg-[#232128] rounded-lg w-16 h-16"></div>
                    <div class="flex flex-col justify-between py-0.5 ml-2">
                        <div class="bg-[#232128] rounded-lg w-30 h-3"></div>
                        <div class="bg-[#232128] rounded-lg w-20 h-3"></div>
                        <div class="bg-[#232128] rounded-lg w-10 h-3"></div>
                    </div>
                </div>
            </div>
            ...
            <div v-else-if="fetchError" class="flex flex-auto">
                <div class="m-auto flex flex-col items-center">
                    <svg class="w-38 h-38" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M4.91003 11.84C9.21003 8.51998 14.8 8.51998 19.1 11.84" stroke="#232128"
                            stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                        <path d="M2 8.35998C8.06 3.67998 15.94 3.67998 22 8.35998" stroke="#232128" stroke-width="1.5"
                            stroke-linecap="round" stroke-linejoin="round" />
                        <path d="M6.79004 15.49C9.94004 13.05 14.05 13.05 17.2 15.49" stroke="#232128"
                            stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
            ...
            <div v-else-if="result.songs.length" class="flex-auto pt-4 mt-4 overflow-auto scrollbar-thin pb-25">
                <div class="flex flex-col gap-4 scroll-smooth" v-memo="[result.songs]">
                    <playListItem v-for="(music, index) in result.songs" @play="play(result.songs, index)"
                        :music="music" :key="index" />
                </div>
            </div>
            <div v-else class="flex flex-auto">
                <div class="m-auto flex flex-col items-center">
                    ...
                    <button @click.once="back" v-wave class="text-[#fcab14] bg-[#fcab14] bg-opacity-10 select-none border-none py-1 rounded px-2 text-sm mt-2">Back</button>
                </div>
            </div>
        </div>
    </div>
