...
<template>
    <div v-if="store.favorites.length" class="flex-auto mt-4 overflow-auto scrollbar-thin pb-25">
        <div class="flex flex-col gap-4 scroll-smooth">
            <playListItem v-for="(music, index) in store.favorites" @play="play(store.favorites ,index)" :music="music" :key="index"/>
        </div>
    </div>
