        ...
        <header class="flex items-center justify-between">
            <div class="flex items-center">
                ...
                <p class="ml-2 text-white">
                    <mark class="bg-transparent text-[#fcab14]">History</mark>
                </p>
            </div>
        </header>
        <!-- body -->
        <section class="mt-4 h-[calc(100%-3rem)] flex-col flex duration-300 transition-all">
            <div v-if="store.history.length" class="flex-auto mt-4 overflow-auto scrollbar-thin pb-25">
                <TransitionGroup name="fade" tag="div" class="flex flex-col gap-4 scroll-smooth">
                    <playListItem v-for="(music, index) in (store.history as any[])" @play="play(store.history, index)"
