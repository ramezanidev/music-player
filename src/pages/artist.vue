...
<template>
  <router-view v-if="isShowArtistPlayList"></router-view>
  <div class="p-4 h-full box-border" v-else>
    <header class="flex items-center justify-between">
      <div class="flex items-center">
        ...
        <p class="ml-2 text-white">
          <mark class="bg-transparent text-[#fcab14]">Trending</mark> Artists
        </p>
      </div>
    </header>
    <div class="flex h-[calc(100%-2rem)] w-full">
      ...
      <div v-if="loading"
        class="grid w-full h-[fit-content] scrollbar-thin grid-cols-3 gap-4 mt-4 animate-pulse overflow-auto scroll-smooth">
        <div v-for="i in 8" :key="i"
          class="w-full h-[fit-content] bg-[#232128] box-border relative pt-[100%] overflow-hidden rounded-full shadow-md">
        </div>
      </div>
      ...
      <div v-else-if="result.length" class="flex-auto mt-4 overflow-auto scrollbar-thin pb-25">
        <div class="grid grid-cols-3 gap-4 scroll-smooth">
          <router-link v-for="(artist, index) in result" :to="`/artist/${artist.id}`" :key="index"
            ...
            <div class="absolute inset-0">
              ...
              <div
                ...
                <p
                  class="text-white text-sm overflow-ellipsis overflow-hidden whitespace-nowrap px-1.5 w-full box-border">
                  {{ artist.fullName }}</p>
                <span class="text-[10px] text-[#fcab14]">{{ Math.round(artist.followersCount / 1000) || 1 }}k</span>
              </div>
            </div>
          </router-link>
        </div>
